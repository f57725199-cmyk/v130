...
export const Store: React.FC<Props> = ({ user, settings, onUserUpdate }) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 pb-10">
      {/* STORE BANNER (Admin Controlled) */}
      {settings?.storeBannerText && (
          <div className="mb-6 bg-gradient-to-r from-purple-600 to-indigo-600 p-6 rounded-3xl shadow-xl flex items-center gap-4 text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 -mr-4 -mt-4 w-24 h-24 bg-white/10 rounded-full blur-2xl"></div>
              <Sparkles className="text-yellow-300 shrink-0" size={32} />
              <div>
                  <p className="font-black uppercase text-[10px] tracking-[0.2em] text-purple-200 mb-1">Exclusive Offer</p>
                  <p className="font-black text-xl leading-tight">{settings.storeBannerText}</p>
              </div>
          </div>
      )}
