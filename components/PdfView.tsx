...
import { FileText, Lock, ArrowLeft, Crown, Star, CheckCircle, AlertCircle } from 'lucide-react';
...
export const PdfView: React.FC<Props> = ({ 
  ...
  const handlePdfClick = (type: 'FREE' | 'PREMIUM') => {
      ...
      if (!link) {
          alert("Coming Soon! This content is being prepared.");
          return;
      }
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 animate-in fade-in slide-in-from-right-8">
       ...
       {activePdf ? (
           <div className="h-[calc(100vh-80px)] w-full bg-slate-100 relative overflow-hidden">
               {/* WATERMARK OVERLAY */}
               {settings?.isWatermarkEnabled && settings.watermarkText && (
                   <div className="absolute inset-0 pointer-events-none z-50 flex flex-wrap content-center justify-center gap-16 overflow-hidden opacity-30 select-none">
                       {/* Generating a pattern of watermarks */}
                       {Array.from({ length: 20 }).map((_, i) => (
                           <div 
                               key={i} 
                               className="bg-black text-gray-400 px-6 py-2 text-xl font-bold rounded transform -rotate-12 whitespace-nowrap shadow-lg"
