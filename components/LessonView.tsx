...
interface Props {
  ...
  loading: boolean;
  onBack: () => void;
  onMCQComplete?: (count: number, answers: Record<number, number>, usedData: MCQItem[]) => void; 
...
export const LessonView: React.FC<Props> = ({ 
  ...
  if ((content.type === 'MCQ_ANALYSIS' || content.type === 'MCQ_SIMPLE') && content.mcqData) {
      ...
      const handleRecreate = () => {
          if (!window.confirm("Re-create (Shuffle) Questions? Progress will be reset.")) return;
          // Shuffle
      ...
      const handleConfirmSubmit = () => {
          ...
          // @ts-ignore
          if (onMCQComplete) onMCQComplete(score, mcqState, displayData);
      ...
      const handleAnalysisPurchase = () => {
           ...
           if (!user || !onUpdateUser) {
               alert("Error: User session missing.");
               return;
           }

           if (user.credits < ANALYSIS_COST) {
               alert(`Insufficient Coins! You need ${ANALYSIS_COST} coins.`);
               return;
           }

           if (!window.confirm(`Unlock Analysis for ${ANALYSIS_COST} Coins?`)) return;
                       ...
                       {showResults ? (
                           ...
                           <div className="flex flex-col items-end">
                               <div className="flex gap-3 text-xs font-bold mb-1">
                                   ...
                                   <span className="text-red-500 flex items-center gap-1"><XCircle size={12}/> {currentWrong}</span>
                               </div>
                               <span className="text-xs text-slate-400">
                                   Batch {batchIndex + 1} • {Object.keys(mcqState).length}/{displayData.length} Attempted
                               </span>
                           </div>
                       )}
                   </div>
